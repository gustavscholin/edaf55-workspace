...
import se.lth.cs.realtime.OngoingThread;
import se.lth.cs.realtime.RTThread;

/**
 * A graphic visualization of a washing machine. Displays the state of the
 * machine in its own window. Runs in its own thread.
 */
class WashingView extends Frame implements Runnable {
     ...
     *            Simulation speed.
     */
    public WashingView(WashingMachineSimulation mach, double speed) {
        ...
        add(myWaterLabel);
        myWaterLabel.setBounds(WATERLABEL_X, WATERLABEL_Y, WATERLABEL_W, WATERLABEL_H);

        myTempLabel = new Label();
        add(myTempLabel);
        myTempLabel.setBounds(TEMPLABEL_X, TEMPLABEL_Y, TEMPLABEL_W, TEMPLABEL_H);

        myTimeLabel = new Label("", Label.CENTER);
        add(myTimeLabel);
        myTimeLabel.setBounds(TIMELABEL_X, TIMELABEL_Y, TIMELABEL_W, TIMELABEL_H);

        myHeatingLabel = new Label("", Label.CENTER);
        add(myHeatingLabel);
        myHeatingLabel.setBounds(HEATINGLABEL_X, HEATINGLABEL_Y, HEATINGLABEL_W, HEATINGLABEL_H);
    ...
    /**
     * Update lock display. Creates a Graphics object g, calls updateLock(g), and
    ...
    /**
     * Update fill tap display. Creates a Graphics object g, calls updateFill(g),
     * and disposes the Graphics object.
     */
    ...
    /**
     * Update drain pump display. Creates a Graphics object g, calls updateDrain(g),
     * and disposes the Graphics object.
     */
    ...
    /**
     * Update tumbler display. Creates a Graphics object g, calls updateTumbler(g),
     * and disposes the Graphics object.
     */
     ...
     * 
     * @param g
     *            The Graphics object provided by AWT.
     */
    public void paint(Graphics g) {

        g.drawImage(myMachineImage, MACHINE_X, MACHINE_Y, Color.lightGray, this);
     ...
     * 
     * @param e
     *            The WindowEvent provided by AWT.
     */
    ...
    /**
     * Handle the thread stuff. Call updatePeriodic every now and then. When things
     * go wrong, be sarcastic.
     */
    public void run() {
        boolean hot;
        boolean wet;
        ...
        try {
            ...
            if (hot) {
                ...
                SarcasticDialog sd = new SarcasticDialog(this, "Sacre Bleu!!!",
                        "Un catastrophe grande de la Feu avec cravatte rouge.", myFireImage, 106, 140, Color.orange,
                        Color.black);
                sd.setVisible(true);
                ...
                SarcasticDialog sd = new SarcasticDialog(this, "Sacre Bleu!!!",
                        "Un catastrophe grande de la Aqua maritime cote d'Azur.", myFishImage, 118, 140, Color.blue,
                        Color.white);
                sd.setVisible(true);
            }
        } catch (Exception e) {
    ...
    /*
     * Update water and temperature indicators, show some rotating clothes (if the
     * machine currently is spinning), and display time. Called periodically.
     *
     * To avoid too much redrawing, the bar sizes are cached and the bars are only
     * redrawn if they have changed in size.
     */
    ...
    private void updateFill(Graphics g) {
        if (myMachine.isFilling())
            g.drawImage(myPipeInActiveImage, FILL_X, FILL_Y, Color.white, this);
        else
            g.drawImage(myPipeInImage, FILL_X, FILL_Y, Color.white, this);
    }
    ...
    private void updateDrain(Graphics g) {
        if (myMachine.isDraining())
            g.drawImage(myPipeOutActiveImage, DRAIN_X, DRAIN_Y, Color.white, this);
        else
            g.drawImage(myPipeOutImage, DRAIN_X, DRAIN_Y, Color.white, this);
    }
    ...
    private void updateTumbler(Graphics g) {
        g.drawImage(myTumblerImages[mySpinPosition], TUMBLER_X, TUMBLER_Y, Color.white, this);
    }
    ...
    private void updateTemperature(Graphics g) {
        ...
        myTempLabel.setText("Temperature: " + wholePart + '.' + decPart + " deg C");
    }
    ...
    private void updateWaterLevel(Graphics g) {
        ...
        myWaterLabel.setText("Water level: " + wholePart + '.' + decPart + " l");
    }
    ...
    private void updateTime() {
        ...
        myTimeLabel.setText("" + tDisplayed / 60 + ":" + (mins / 10) + (mins % 10));
    }
